...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Calculates and visualizes the correlation between group abundances across various conditions within an AnnData object.')

    parser.add_argument('--adata', type=str, required=True, help='Path to an AnnData object file containing the dataset to be visualized.')
    parser.add_argument('--groupBy', type=str, required=True, help="The column in `adata.obs` used for defining groups.")
    parser.add_argument('--condition', type=str, required=True, help="The column in `adata.obs` that distinguishes different conditions or samples.")
    parser.add_argument('--normalize', action='store_true', help="Apply z-score normalization to the group counts across conditions. Defaults to False.")
    parser.add_argument('--subsetGroups', type=str, nargs='+', default=None, help="A list specifying a subset of groups to include in the analysis.")
    parser.add_argument('--orderRow', type=str, nargs='+', default=None, help="Custom order for the rows in the heatmap.")
    parser.add_argument('--orderColumn', type=str, nargs='+', default=None, help="Custom order for the columns in the heatmap. Equivalent to `orderRow` due to the square nature of the correlation matrix but kept for consistency.")
    parser.add_argument('--clusterRows', action='store_true', help="Whether to apply hierarchical clustering to rows. Defaults to True unless --no-clusterRows is specified.")
    parser.add_argument('--no-clusterRows', action='store_false', dest='clusterRows', help="Do not cluster rows.")
    parser.add_argument('--clusterColumns', action='store_true', help="Whether to apply hierarchical clustering to columns. Defaults to True unless --no-clusterColumns is specified.")
    parser.add_argument('--no-clusterColumns', action='store_false', dest='clusterColumns', help="Do not cluster columns.")
    parser.add_argument('--cmap', type=str, default='vlag', help="The colormap for the heatmap. Defaults to 'vlag'.")
    parser.add_argument('--figsize', type=float, nargs=2, default=None, help="The size of the figure to create (width, height). If None, the size is inferred.")
    parser.add_argument('--overlayValues', action='store_true', help="Overlay the actual correlation values on the heatmap.")
    parser.add_argument('--fontSize', type=int, default=10, help="Font size for overlay values. Defaults to 10.")
    parser.add_argument('--fontColor', type=str, default='black', help="Color of the font used for overlay values. Defaults to 'black'.")
    parser.add_argument('--fileName', type=str, default='groupCorrelation.pdf', help="Name of the file to save the heatmap. Defaults to 'groupCorrelation.pdf'.")
    parser.add_argument('--saveDir', type=str, default=None, help="Directory to save the generated heatmap. If None, the heatmap is not saved.")
    
    args = parser.parse_args()
    
    # Execute groupCorrelation with the provided arguments
    groupCorrelation(adata=args.adata, 
