        ...
        for i in range(corr_matrix.shape[0]):
            for j in range(corr_matrix.shape[1]):
                text = plt.text(j, i, f"{corr_matrix[i, j]:.2f}",
                               ha="center", va="center", color=fontColor,fontsize=fontSize)

    plt.xticks(ticks=np.arange(len(col_order)), labels=np.array(var_names)[col_order], rotation=90)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Compute and visualize the correlation among markers within an AnnData object.')
    ...
    parser.add_argument('--layer', type=str, default='log', help="Specifies the layer of `adata` to use for the correlation analysis. Defaults to 'log'.")
    parser.add_argument('--subsetMarkers', type=str, nargs='+', default=None, help="A list of marker genes or features to include in the correlation analysis.")
    parser.add_argument('--orderRow', type=str, nargs='+', default=None, help="Custom order for the rows based on marker names.")
    parser.add_argument('--orderColumn', type=str, nargs='+', default=None, help="Custom order for the columns based on marker names.")
    parser.add_argument('--clusterRows', action='store_true', help="Whether to cluster rows. Defaults to True unless --no-clusterRows is specified.")
    parser.add_argument('--no-clusterRows', action='store_false', dest='clusterRows', help="Do not cluster rows.")
    parser.add_argument('--clusterColumns', action='store_true', help="Whether to cluster columns. Defaults to True unless --no-clusterColumns is specified.")
    ...
    parser.add_argument('--figsize', type=float, nargs=2, default=None, help="The size of the figure to create. Specify width and height.")
    parser.add_argument('--overlayValues', action='store_true', help="Overlay the actual correlation values on the heatmap.")
    parser.add_argument('--fontSize', type=int, default=10, help="Font size for the overlay values. Defaults to 10.")
    parser.add_argument('--fontColor', type=str, default='black', help="Color of the font used for overlay values. Defaults to 'black'.")
    parser.add_argument('--fileName', type=str, default='markerCorrelation.pdf', help="Name of the file to save the heatmap. Defaults to 'markerCorrelation.pdf'.")
    ...
    markerCorrelation(adata=args.adata, 
                      layer=args.layer, 
