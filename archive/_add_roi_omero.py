...
Example:
        ...
        adata = add_roi_omero(adata, roi=roi_df, label='Sample_ROI')
    
        # Add ROIs to a specific image in a dataset containing multiple images
        adata = add_roi_omero(adata, roi=roi_df, imageid='image_column', subset=['image_01'], label='Image1_ROI')
    
        # Append multiple ROI datasets to the same column in AnnData without overwriting
        adata = add_roi_omero(adata, roi=roi_df_first_set, label='Combined_ROI', overwrite=False)
        adata = add_roi_omero(adata, roi=roi_df_second_set, label='Combined_ROI', overwrite=False)
